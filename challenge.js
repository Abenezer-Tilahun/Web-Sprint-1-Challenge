 // In CHALLENGES 1-4 you will write JavaScript functions.
    ...
    function concatenator(a,b) {
        ...
        if(typeof a !== typeof b) return "a and b must be of the same type"
        if(typeof a !== 'string' && typeof a !== 'number' || typeof b!=='string' && typeof b!=='number') return "a and b must be numbers or strings"
      ...
      function falsinessDetector(a,b,c,d,e) {
        ...
        if (a && b && c && d && e) return "all values truthy";
        if (!a && !b && !c && !d && !e) return "all values falsy";
      ...
      function triangleAnalyzer(angleA, angleB,angleC) {
        ...
        // ❗ Hint: It's advised to deal with the other conditions first, like arguments being of the wrong type.
        //          In the real world, these are considered "edge cases".
       if(typeof angleA !== 'number' || typeof angleB !== 'number' || typeof angleC !== 'number') return "all angles must be numbers"
       if(angleA <= 0 || angleB <= 0 || angleC <= 0) return "all angles must be greater than zero"
       if(angleA+angleB+angleC !== 180) return "angles must total 180"
       if(angleA === 90 || angleB === 90 || angleC === 90) return "this is a right triangle"
       if(angleA > 90 || angleB > 90 || angleC > 90) return "this is an obtuse triangle"
       if(angleA < 90 && angleB < 90 && angleC < 90) return "this is an acute triangle"
      ...
      function advancedBatteryCare(percentage) {
        ...
        if(percentage === 0) return "congrats, battery dead"
        if(percentage === 100) return "congrats, you cooked your battery"
        if(percentage < 20) return "please connect charger"
        if(percentage > 80) return "please unplug charger"
        if(percentage >= 20 && percentage <= 80) return `your battery is at ${percentage}%`
      }
      ...
      document.addEventListener('DOMContentLoaded', function() {
        batteryLevel.addEventListener('input', function() {
                  const batteryLevel = this.value;
                  const batteryStatus = advancedBatteryCare(Number(batteryLevel));
                  batteryStatus.innerText = batteryStatus;
              });
          });
